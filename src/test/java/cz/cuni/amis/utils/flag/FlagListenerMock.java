...
import cz.cuni.amis.utils.HashMapList;
...
public class FlagListenerMock<T> implements FlagListener<T> {
	...
	@Override
	public void flagChanged(T oldValue, T changedValue) {
	...
	public void checkValuesSubset(String prefix, T[] check) {
		HashMapList<T, T> all = new HashMapList<T, T>();
		for (T value : values) {
			all.get(value).add(value);
		}
	...
	public void checkValuesAnyExceptSubset(String prefix, T[] check) {
		HashMapList<T, T> all = new HashMapList<T, T>();
		for (T value : values) {
			all.get(value).add(value);
		}
		for (T value : check) {
			if (all.get(value).size() != 0) {
				System.out.println(prefix + " [CHECK-ERROR]: invalid values found '" + value + "'");
				throw new RuntimeException("check failed");
			}			
		}		
		clearValues();
